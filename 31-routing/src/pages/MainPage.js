...
/*
  ...
  setSearchParams 는 쿼리 파라미터를 수정하는데 사용되는 함수

  searchParams.get('mode') 는 현재 URL의 쿼리 스트링에서 mode라는 이름의 파라미터 값을 가져옴

  버튼 클릭 이벤트 핸들러는 setSearchParams({mode : 'Dark'})를 호출하여 쿼리 스트링의 mode 파라미터를 'Dark'로 설정
