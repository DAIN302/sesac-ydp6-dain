...
var x = 20;
console.log(x);
// 동일한 변수명을 가진 변수를 한 스코프 내에서 여러 번 선언하는 것
// 이전에 선언된 변수값을 무시하게 됨
// 마지막에 할당된 값이 저장되면서 코드 혼동시킬 수 있음

// ** 재선언
var y = 10;
var y = 20;
console.log(y)
// 이미 선언된 변수를 같은 스코프 내에서 다시 선언하는 것
...
// 1. 키워드 (예약어) 사용 불가능
// 키워드란?
// 특별한 역할을 이미 하고 있는 단어(이미 자바스크립트에서 사용된 예약어들은 변수명으로 사용하지 못함)
// ex) let, var, const, return, void, for, while, class 
// 2. 숫자로 시작 x
// 3. 특수문자 사용 불가 : 예외) _(언더바) $(달러)
// 4. 공백문자 사용 불가
// 식별자에 단어 두개 이상 작성하고 싶은 경우 -> camelCase 나 snakeCase 사용  ex) redApple, red_apple

// 상수란?
// 프로그램이 실행되는 동안 값이 변하지 않는 데이터
// 한번 정의되면 프로그램 실행 중에는 변경되지 않아야함
// 보통 대문자로 표기(보통 개발자들 국룰) ex) APPLE, BANANA, CHERRY
