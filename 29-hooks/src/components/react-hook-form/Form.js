      ...
      <form onSubmit={handleSubmit(onValid, onInValid)}>
        <div>
          <input
            ...
            {...register("username", {
              // register를 'username'으로 해줘서 name이 username됨
...
/*
{...register()} -> 왜 이렇게 사용?
리액트 컴포넌트에서 ...연산자는 객체나 배열을 펼처서(spread) 각각의 요소나 속성을 개별적인 prop으로 전달할 떄 사용
이 문법을 사용하면 코드가 간결해지고 여러개의 props을 한 번에 전달 가능


props?
컴포넌트에 전달되는 입력 값
부모 컴포넌트가 자식 컴포넌트에 데이터를 전달 할 때 사용
이 props 를 통해 컴포넌트의 동작이나 출력이 결정

리액트에서는 객체의 속성을 컴포넌트의 props로 전달할 때, 각 속성을 개별적으로 전달 가능

register 함수는 입력 필드를 폼 상태에 등록하고 제어하는 데 필요한 여러 속성을 포함하는 객체를 반환
이 객체를 input 컴포넌트에 전달하기 위해서 스프레드 연산자를 사용!
